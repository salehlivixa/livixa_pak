...
public class RestClient {
	...
	public RestClient() {
		...
		// .addHeader("X-API-AUTHID",
		// AppPreference.getValue(SampleApplication.getAppContext(),
		// AppKeys.KEY_X_API_AUTHID))
		// .addHeader("X-API-AUTHKEY",
		// AppPreference.getValue(SampleApplication.getAppContext(),
		// AppKeys.KEY_X_API_AUTHKEY))
		// .build();
	...
	public RestClient(boolean headers) {
		...
		Retrofit restAdapter = new Retrofit.Builder()
				.baseUrl(AppWebServices.BASE_URL).client(httpClient)
		...
		public Response intercept(Chain chain) throws IOException {
			...
			String responseLog = String.format(
					"Received response for %s in %.1fms%n%s", response
							.request().url(), (t2 - t1) / 1e6d, response
							.headers());
