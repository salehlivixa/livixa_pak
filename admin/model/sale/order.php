...
class ModelSaleOrder extends Model
{
    public function getOrder($order_id)
    {
        ...
        if ($order_query->num_rows) {
            ...
            $sql = "Select status from order_details_mobile_api where ref_order_id=" . $order_id;

            $get_status = $link->query($sql);
            $status = [];
            foreach ($get_status as $status) {
                $status = $status['status'];
            }
            $sql = "Select * from order_mac_addresses where order_id=" . $order_id;

            $get_macs = $link->query($sql);
            $mac_count = mysqli_num_rows($get_macs);

            $sql = "Select quantity from ocn8_order_product where order_id=" . $order_id;

            $mac_quantities = $link->query($sql);
            $mac_quantity = 0;
            foreach ($mac_quantities as $quantity) {
                $mac_quantity = $mac_quantity + $quantity['quantity'];
            }
