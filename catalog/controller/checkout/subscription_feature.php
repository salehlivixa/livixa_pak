...
class Controllercheckoutsubscriptionfeature extends Controller
{
    public function index()
    {
        ...
        $get_packages = $link->query($packages);
        foreach ($get_packages as $get_package) {
            $sub_pack['package'][] = $get_package['package'];
        }

        $subscriptions = '';
        $subscription = [];
        $subscriptions = "Select * from subscriptions";
        $get_subscriptions = $link->query($subscriptions);
        foreach ($get_subscriptions as $get_subscription) {
            $sub_pack['subscription'][] = $get_subscription['subscription'];
        }

        $this->response->setOutput($this->load->view('checkout/subscription_feature'),$sub_pack);
    }
